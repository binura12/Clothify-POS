...
public class ResetPasswordFormController implements Initializable {
    ...
    void btnResetOnAction(ActionEvent event) {
        ...
        if (!PasswordValidateUtil.isValidPassword(password)){
            ...
            boolean isPasswordReset = cashierService.resetPassword(txtGmailAddress.getText(), txtPassword.getText());
            if (isPasswordReset) {
    ...
    void btnSendOnAction(ActionEvent event) {
        ...
        CashierService cashierService = ServiceFactory.getInstance().getServiceType(ServiceType.cashier);
        boolean isForgetPswEmailFound = cashierService.forgotPassword(enteredGmail);
        if (isForgetPswEmailFound) {
            otp = cashierService.generateOTP();
